...
function CheckCashStreamPrivilege($id, $gp_id)
{
    ...
    $sql = "select count(*) from stream where id = ? and gp_id = ? and is_delete=0";
...
function GetCashStreamRecord($id)
{
    ...
    if( $qresult = $db->query($sql) )
    {
        if( $row = $qresult->fetch_assoc() )
        {
            ...
            $row['name'] = filter_htmlspecialchars($row['name']);
...
{
    $sql = "insert into stream (name, date, sum, intro, record_person_id, cat_id, dire_type, type, gp_id) ".
        "values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if( !$stmt->bind_param('sidsiiiii', '', $date, $sum, $intro, $user_id, $cat_id, $type, $pay_method, $gp_id) )
    {
        echo "Database Error:" . $stmt->error;
        exit(0);
    }
...
{
    $sql = "update stream set name = ?, date = ?, sum = ?, intro = ?, cat_id = ?, dire_type = ?, type = ? ".
    ...
    if( !$stmt->bind_param('sidsiiiii', '', $date, $sum, $intro, $cat_id, $type, $pay_method, $id, $gp_id) )
    {
        echo "Database Error:" . $stmt->error;
        exit(0);
    }
